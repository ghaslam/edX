...
def buildCoder(shift):
    ...
    Dict1 = dict(zip(upper,shifted_upper))
    Dict2 = dict(zip(lower,shifted_lower))
    letterDict = {}
    letterDict.update(Dict1)
    letterDict.update(Dict2)
    return letterDict
...
def applyCoder(text, coder):
    ...
    for char in text:
        if char in coder.keys():
            scramble.append(coder.get(char))
        else:
...
def applyShift(text, shift):
    ...
    Caesar = applyCoder(text, buildCoder(shift))
    return Caesar
...
def decryptStory():
    ...
    print bestShift    
    decrypt = applyShift(text, bestShift)
    print decrypt
